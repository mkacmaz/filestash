...
func sendFile(res http.ResponseWriter, file io.ReadCloser, fileName string) {
	body := &bytes.Buffer{}
	...
	r, err := http.NewRequest("POST", "http://SERVER_IP/api/v1/uploads", body)
	if err != nil {
		SendErrorResult(res, err)
	}
